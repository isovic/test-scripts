...
def RUN_CONSENSUS_TEST_ECOLINMETH():
	...
	run_all_mappers_only(('%s/../data/nmeth/reference/escherichia_coli.fa' % SCRIPT_PATH), ('%s/../data/nmeth/reads/reads-nmeth-all_2d.fastq' % SCRIPT_PATH), 'nmeth-all_2d', '%s/../data/out/consensus-nmeth-all_2d/' % (SCRIPT_PATH), 'nanopore');
	evaluate_alignments(('%s/../data/nmeth/reference/escherichia_coli.fa' % SCRIPT_PATH), ('%s/../data/nmeth/reads/reads-nmeth-all_2d.fastq' % SCRIPT_PATH), 'nmeth-all_2d', '%s/../data/out/consensus-nmeth-all_2d/' % (SCRIPT_PATH), cov_threshold=10);
	evaluate_unique_alignments(('%s/../data/nmeth/reference/escherichia_coli.fa' % SCRIPT_PATH), ('%s/../data/nmeth/reads/reads-nmeth-all_2d.fastq' % SCRIPT_PATH), 'nmeth-all_2d', '%s/../data/out/consensus-nmeth-all_2d/' % (SCRIPT_PATH), cov_thresholds=[20, 15, 10]);
	collect_alignments(('%s/../data/nmeth/reference/escherichia_coli.fa' % SCRIPT_PATH), ('%s/../data/nmeth/reads/reads-nmeth-all_2d.fastq' % SCRIPT_PATH), 'nmeth-all_2d', '%s/../data/out/consensus-nmeth-all_2d/' % (SCRIPT_PATH), [20, 15, 10]);
...
def RUN_CONSENSUS_TEST_BE1():
	...
	dataset_name = 'be1-all1d2d';
	out_path = '%s/../data/out/consensus-%s/' % (SCRIPT_PATH, dataset_name);
	mappers_to_run = ['daligner', 'graphmap', 'graphmap-anchor', 'last', 'bwamem', 'blasr', 'marginalign', 'marginaligngraphmap', 'marginaligngraphmap-anchor']

	run_all_mappers_only(reference_path, reads_path, dataset_name, out_path, 'nanopore', do_not_recalc=True, is_circular=True, select_mappers=mappers_to_run);
	evaluate_alignments(reference_path, reads_path, dataset_name, out_path, cov_threshold=20);
	evaluate_alignments(reference_path, reads_path, dataset_name, out_path, cov_threshold=15);
	evaluate_alignments(reference_path, reads_path, dataset_name, out_path, cov_threshold=10);
	collect_alignments(reference_path, reads_path, dataset_name, out_path, cov_thresholds=[20, 15, 10]);
...
def RUN_CONSENSUS_TEST_MAP006():
        ...
        reads_path = ('%s/../data/consensus-ecoliMAP006/reads/reads-MAP006.fastq' % SCRIPT_PATH)
        dataset_name = 'ecoliMAP006';
        out_path = '%s/../data/out/consensus-%s/' % (SCRIPT_PATH, dataset_name);
        mappers_to_run = ['daligner', 'graphmap', 'graphmap-anchor', 'last', 'bwamem', 'blasr', 'marginalign', 'marginaligngraphmap', 'marginaligngraphmap-anchor']
...
def RUN_DRAFT_ASSEMBLY_REFERENCE_TESTS():
	...
	# run_all_mappers_only(assembly_draft, reads_path, dataset_name, out_path, 'nanopore', do_not_recalc=True, is_circular=True,
	# 					select_mappers=['daligner', 'graphmap', 'graphmap-anchor', 'last', 'bwamem', 'blasr', 'marginalign', 'marginaligngraphmap', 'marginaligngraphmap-anchor']);
	evaluate_consensus_sequences(reference_path, assembly_draft, dataset_name, out_path, 10);
	evaluate_consensus_sequences(reference_path, assembly_draft, dataset_name, out_path, 20);
	# evaluate_unique_alignments(assembly_draft, reads_path, dataset_name, out_path);
	### collect_alignments(assembly_draft, reads_path, dataset_name, out_path);
	### collect_unique_alignments(assembly_draft, reads_path, dataset_name, out_path);
...
def RUN_SV_TEST():
	...
	# run_all_mappers_only(original_reference, reads_path, 'all_2d_for_sv', '%s/../data/out/all_2d_for_sv-normal_ref/' % (SCRIPT_PATH),
	# 			'nanopore', do_not_recalc=True, is_circular=True,
	# 			select_mappers=['daligner', 'graphmap', 'graphmap-anchor', 'last', 'bwamem', 'blasr', 'marginalign', 'marginaligngraphmap', 'marginaligngraphmap-anchor']);
	# ### Second, run the mappers on the modified reference.
	# run_all_mappers_only(mutated_reference, reads_path, 'all_2d_for_sv', '%s/../data/out/all_2d_for_sv-indel_ref/' % (SCRIPT_PATH),
	# 			'nanopore', do_not_recalc=True, is_circular=True,
	# 			select_mappers=['daligner', 'graphmap', 'graphmap-anchor', 'last', 'bwamem', 'blasr', 'marginalign', 'marginaligngraphmap', 'marginaligngraphmap-anchor']);
	...
	files_for_blacklist = find_files('data/out/all_2d_for_sv-normal_ref/analysis-final/', '*chained.csv', -1);
	# files_for_blacklist = find_files('data/out/all_2d_for_sv-normal_ref/analysis-final/', '*structvars.csv', -1);
	...
	full_truth_events_path = 'data/out/all_2d_for_sv-indel_ref/truth_events/truth_events_and_blacklist.csv';
	...
	files_for_eval = find_files('data/out/all_2d_for_sv-indel_ref/analysis-final/', '*chained.csv', -1);
	# files_for_eval = find_files('data/out/all_2d_for_sv-indel_ref/analysis-final/', '*structvars.csv', -1);
...
def RUN_AMPLICON_TEST():
	...
	# run_all_mappers_only(reference, reads, dataset_name, out_path, 'nanopore', do_not_recalc=False, is_circular=False,
	#                                               select_mappers=['daligner', 'graphmap', 'graphmap-anchor', 'last', 'bwamem', 'blasr', 'marginalign', 'marginaligngraphmap', 'marginaligngraphmap-anchor']);
...
# gi|224589814|ref|NC_000022.10| Homo sapiens chromosome 22, GRCh37.p13 Primary Assembly
	...
	sam_files = [
	                ...
	                '%s/GraphMap-amplicons-f1000-2d.sam' % (out_path),
	                '%s/GraphMap-anchor-amplicons-f1000-2d.sam' % (out_path),
	                '%s/LAST-amplicons-f1000-2d.sam' % (out_path),
	                '%s/BWAMEM-amplicons-f1000-2d.sam' % (out_path),
	                '%s/BLASR-amplicons-f1000-2d.sam' % (out_path),
	                '%s/marginAlign-amplicons-f1000-2d.sam' % (out_path),
	                '%s/marginAlignGraphMap-amplicons-f1000-2d.sam' % (out_path),
	                '%s/marginAlignGraphMap-anchor-amplicons-f1000-2d.sam' % (out_path),
	                '%s/DALIGNER-amplicons-f1000-2d.sam' % (out_path),
					];
...
def copy_from_server(alignments_path, cov_threshold=20):
	alignments_path = os.path.abspath(alignments_path).split('graphmap-reproduce/')[-1];
	vcf_folder_local = '%s/analysis-intermediate' % (alignments_path);
	vcf_folder_remote = '/home/isovic/graphmap/git/graphmap-reproduce/%s/analysis-intermediate' % (alignments_path);
	execute_command('mkdir -p %s' % (vcf_folder_local));
	execute_command('scp -P 2222 isovic@assembly.zesoi.fer.hr:%s/consensus-*-cov_%d*.vcf %s' % (vcf_folder_remote, cov_threshold, vcf_folder_local));

	# execute_command('mkdir -p %s/../data/out/mutated_ref_draftlike_ecoliR7.3/analysis-intermediate/' % (SCRIPT_PATH));
	# execute_command('scp -P 2222 isovic@genome.zesoi.fer.hr:/home/isovic/graphmap/git/graphmap-reproduce/data/out/mutated_ref_draftlike_ecoliR7.3/analysis-intermediate/consensus-*.vcf %s/../data/out/mutated_ref_draftlike_ecoliR7.3/analysis-intermediate/' % (SCRIPT_PATH));

	# execute_command('mkdir -p %s/../data/out/mutated_ref_draftlike_ecolinmeth/analysis-intermediate/' % (SCRIPT_PATH));
	# execute_command('scp -P 2222 isovic@genome.zesoi.fer.hr:/home/isovic/graphmap/git/graphmap-reproduce/data/out/mutated_ref_draftlike_ecolinmeth/analysis-intermediate/consensus-*.vcf %s/../data/out/mutated_ref_draftlike_ecolinmeth/analysis-intermediate/' % (SCRIPT_PATH));

	# execute_command('scp -P 2222 isovic@genome.zesoi.fer.hr:/home/isovic/graphmap/git/graphmap-reproduce/data/out/mutated_ref_draftlike_ecoliR7.3/analysis-intermediate/consensus-DALIGNER*.vcf %s/../data/out/mutated_ref_draftlike_ecoliR7.3/analysis-intermediate/' % (SCRIPT_PATH));
	# execute_command('scp -P 2222 isovic@genome.zesoi.fer.hr:/home/isovic/graphmap/git/graphmap-reproduce/data/out/mutated_ref_draftlike_ecoliR7.3/analysis-intermediate/consensus-GraphMap*.vcf %s/../data/out/mutated_ref_draftlike_ecoliR7.3/analysis-intermediate/' % (SCRIPT_PATH));

	# execute_command('scp -P 2222 isovic@genome.zesoi.fer.hr:/home/isovic/graphmap/git/graphmap-reproduce/data/out/mutated_ref_draftlike_ecoliR7.3/analysis-intermediate %s/../data/out/mutated_ref_draftlike_ecoliR7.3/analysis-intermediate' % (SCRIPT_PATH));
# '	 %s/analysis-intermediate/consensus-DALIGNER-%s-cov_20.variant.vcf' % (alignments_path, dataset_name));
...
def collect_consensus_sequence_comparison_results(dnadiff_path, out_collect_file):
	...
	for report_file in report_files:
		...
		if ('BLASR' in mapper_name):
			...
			mapper_name = 'BWA-MEM';
		elif (('ref_vs_draftasm' in mapper_name)):
...
def evaluate_consensus_sequences(reference_file, assembly_sequence_file, dataset_name, alignments_path, cov_threshold=20):
	...
	# dnadiff_prefix = '%s/dnadiff_cov_%d/' % (alignments_path);
	# collect_consensus_sequence_comparison_results(dnadiff_prefix, '%s/collected-dnadiff.csv' % (dnadiff_prefix));

	# return;
	
	# copy_from_server(alignments_path, cov_threshold);
	# return;

	dnadiff_prefix = '%s/dnadiff_cov_%d/%s/%s' % (alignments_path, cov_threshold, 'ref_vs_draftasm', 'ref_vs_draftasm');
